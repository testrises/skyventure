...
describe('Sky ventures CRUD API', () => {
 ...
 it('should create a project', async () => {
  ...
  it('should edit a task', async () => {
    ...
    it('should fetch projects that belongs to a user', async () => {
         const response = await request(app)
          .get('/api/project/672680da0ee6830ca45b095f/list')
          .set('Authorization', `Bearer ${valid_token}`)
          expect(response.status == 400 || response.status == 200 || response.status == 401).toBe(true);
      });
  

      it('should fetch a particular project', async () => {
        const response = await request(app)
         .get('/api/project/6726867c13c4e6d77cb53deb')
         .set('Authorization', `Bearer ${valid_token}`)
         expect(response.status == 400 || response.status == 200 || response.status == 401).toBe(true);
     });

     it('should fetch tasks that belongs to a user in a particular project', async () => {
      const response = await request(app)
       .get('/api/task?project_id?=6726867c13c4e6d77cb53deb')
       .set('Authorization', `Bearer ${valid_token}`)
       expect(response.status == 400 || response.status == 200 || response.status == 401).toBe(true);
     });


     it('should mass update task for a project', async () => {
      const response = await request(app)
       .put('/api/task/mass-update/6726867c13c4e6d77cb53deb/completed')
       .set('Authorization', `Bearer ${valid_token}`)
       expect(response.status == 400 || response.status == 200 || response.status == 401).toBe(true);
     });
